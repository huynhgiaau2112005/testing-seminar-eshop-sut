...
> Phạm vi an toàn: toàn bộ lệnh tác động dữ liệu trong tài liệu chỉ dùng cho **SQLite local** `backend/database.sqlite` và các database MongoDB local `eshop_nosql_lab`/`seminar_demo`. `reset-db`, `db.dropDatabase()` và `npm run db:seed` có thể xóa dữ liệu local hiện có. Không dùng các lệnh này với dữ liệu thật/production.
...
| `migrate-mongo` (cài cùng `backend` dependencies)            | migration MongoDB         | `cd backend && npx migrate-mongo status`                        |
| Docker Desktop + MongoDB (hoặc MongoDB Community + mongosh)   | demo NoSQL local ở phần 8 | `docker --version`; `docker exec mongodb mongosh --version`     |
...
flowchart LR
    ...
    H --> I[Migrate-mongo migration]
```

_Hình 2 — Demo NoSQL tương ứng 8 đầu việc: validation, dữ liệu giả, failure có chủ đích, masking, query và migration bằng migrate-mongo._

### 8.1 Phase 1 — Khởi động MongoDB local
...
Lệnh trên mở MongoDB tại `mongodb://localhost:27017`. Nếu container `mongodb` đã tồn tại nhưng đang dừng, dùng `docker start mongodb`; không chạy lại `docker run` với cùng tên. Có thể thay Docker bằng MongoDB Community Server và mongosh cài local, nhưng toàn bộ lệnh bên dưới giả định URI local `mongodb://localhost:27017`.

**Quan trọng:** chỉ dùng mapping port `27017:27017` khi lệnh `lsof` không cho thấy một `mongod` local đang nghe ở port `27017`. Nếu `mongod` native đang chạy, dừng service đó trước khi làm lab Docker. Trên một số máy, Docker vẫn có thể tạo mapping IPv6 trong khi `mongod` native giữ IPv4; khi đó Node.js/migrate-mongo có thể ghi vào MongoDB native, còn `docker exec mongodb ...` đọc database trong container — hai nơi khác nhau. Nếu không thể dừng service native, dùng một host port trống (ví dụ `-p 27018:27017`) và thay **toàn bộ** URI host trong phần 8 bằng `mongodb://localhost:27018`.
...
```bash
docker exec -it mongodb mongosh "mongodb://localhost:27017/eshop_nosql_lab"
```
...
mongoimport \
  --uri "mongodb://localhost:27017/eshop_nosql_lab" \
  --collection users \
  --jsonArray \
...
### 8.6 Migration MongoDB bằng migrate-mongo

**Khác với SQLite:** migration NoSQL **không dùng Flyway**. MongoDB dùng [`migrate-mongo`](https://github.com/seppevs/migrate-mongo), đã được khai báo trong `backend/package.json`. Cấu hình là [`backend/migrate-mongo-config.js`](backend/migrate-mongo-config.js), migration hiện có là [`backend/migrations/20260720135050-rename_image_url_to_picture_path.js`](backend/migrations/20260720135050-rename_image_url_to_picture_path.js).

Migration này chạy trên database `seminar_demo` theo cấu hình hiện tại và lưu lịch sử trong collection `changelog`. Tại `backend/`, cài dependency, seed dữ liệu mẫu MongoDB, xem trạng thái rồi áp dụng migration:

```bash
npm ci
export MONGO_URI="mongodb://localhost:27017/seminar_demo"
npm run db:seed
...
Kết quả pass: `status` cho thấy migration `20260720135050-rename_image_url_to_picture_path.js` đã được áp dụng và collection `changelog` có lịch sử tương ứng; các documents trong `seminar_demo.users` đổi trường `imageUrl` thành `picturePath`. Kiểm tra bằng:

```bash
docker exec -it mongodb mongosh "mongodb://localhost:27017/seminar_demo" --eval 'db.users.find({}, { imageUrl: 1, picturePath: 1, _id: 0 })'
```

Để mô phỏng rollback, chạy tại `backend/`:
...
Sau `down`, trường `picturePath` được đổi lại thành `imageUrl`. Không chạy migration MongoDB bằng `backend/flyway.conf`: file này chỉ dành cho SQLite và SQL migrations. Với migration phá hủy dữ liệu, phải backup trước và kiểm thử restore; `migrate-mongo` không tự khôi phục dữ liệu đã xóa.
...
| Lab MongoDB             | Không                   | Node.js + Faker sinh dữ liệu; MongoDB validation/masking/query và migration `migrate-mongo` chạy xác định                                                   |
...
| `Ports are not available ... 27017 ... bind: address already in use`          | một process/container khác đang chiếm host port `27017`                                          | dừng service/container đang chiếm cổng hoặc chọn port trống, ví dụ `-p 27018:27017`, rồi thay URI host trong phần 8 thành `mongodb://localhost:27018`; giữ `localhost:27017` cho lệnh chạy _bên trong_ container                                                                        |
...
| `npx migrate-mongo ...` không kết nối được                                    | MongoDB chưa chạy hoặc `MONGO_URI` không trỏ tới `seminar_demo`                                  | chạy `docker ps --filter name=mongodb`; tại `backend/` export lại `MONGO_URI="mongodb://localhost:27017/seminar_demo"`, rồi chạy `npx migrate-mongo status`                                                                                                                            |
...
- [ ] Đã chạy migration MongoDB bằng `migrate-mongo up`, kiểm tra `picturePath`, rồi chạy `migrate-mongo down` để kiểm chứng rollback.
...
- [migrate-mongo](https://github.com/seppevs/migrate-mongo) — cấu hình, `status`, `up`, `down` và collection lưu changelog cho migration MongoDB.
